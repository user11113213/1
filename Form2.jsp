    		...
    		<table>
	    		<tr>
					<td colspan="7"
						...
						<a href="writeAdminBoard.do">게시판 등록</a>
					</td>
				</tr>
	    		<tr>
	    			<th><input type="checkbox" name="user" value="allchk" onclick="allchk(this)"></th>
	    			<th>번호</th>
	    			<th>작성자</th>
	    			<th width="50%">글제목</th>
	    			<th>작성일</th>
		    	...
		    	<c:forEach var="board" items="${boardList}">
				    <tr>
		        		<td style="align:center;"><input type="checkbox" name="user" value="" ></td>
				        <td>${board.code}</td>
		        		<td>${board.name}</td>
				        <td>${board.title}</td>
				        <td>${board.reg_date}</td>
				        <td><a href="adminBoardDetail.do?code=${board.code}">상세</a></td>
				        <td><a href="updateBoardAdmin.do?code=${board.code}">수정</a></td>
			    		<td><a href="deleteBoardAdmin.do?code=${board.code}">삭제</a></td>
				    </tr>
				</c:forEach>
		    </table>
		<form action="multiDelete.do" name="frm">
		    <div style="float: right;">
				<input type="submit" value="삭제">
			</div>
    	</form>
    	<div>
			<form action="searchBoardAdmin.do">
				<select name="column">
